...
class DetailedLogHelper
{
    ...
    /**
     * Silently log an error to PHP error log and application log without throwing or disrupting requests.
     */
    public static function logError(string $context, \Throwable $e): void
    {
        try {
            $message = sprintf(
                '[DetailedLog] %s: %s in %s:%d',
                $context,
                $e->getMessage(),
                $e->getFile(),
                $e->getLine()
    ...
    public static function getEventCategory(EventLogEntry|EmailLogEntry $entry): array
    {
        try {
            ...
            self::logError('Failed determining event category', $e);
            return [
                'category' => 'general',
                'name' => self::translate('plugins.generic.detailedLog.category.general', [], 'General'),
                'icon' => '📌',
    ...
    public static function formatFileStage(?int $fileStage): string
    {
        ...
        try {
            ...
            self::logError("Failed formatting file stage {$fileStage}", $e);
            return "Stage {$fileStage}";
        }
    }
    ...
    public static function formatStageId(?int $stageId): string
    {
        ...
        try {
            ...
            self::logError("Failed formatting stage ID {$stageId}", $e);
    ...
    public static function formatDecision(?string $decision): array
    {
        ...
        try {
            ...
            self::logError('Failed formatting decision', $e);
            return [
    ...
    public static function getRawSettings(int $logId): array
    {
        ...
        try {
            ...
            self::logError("Failed fetching event_log_settings for log_id {$logId}", $e);
            return [];
        }
    }
    ...
    public static function preloadSettings(array $logIds): void
    {
        ...
        try {
            ...
            self::logError('Failed preloading event_log_settings', $e);
        }
    }
    ...
    public static function interpretSetting(string $name, ?string $value, ?string $locale = null): array
    {
        try {
            ...
            self::logError("Failed interpreting setting {$name}", $e);
            return [
                'name' => $name,
                'label' => $name,
                'raw_value' => $value,
    ...
    public static function getAllSubmissionLogIds(int $submissionId): array
    {
        try {
            ...
            self::logError("Failed fetching submission log IDs for submission {$submissionId}", $e);
            return [];
        }
    }
    ...
    public static function getStructuredHighlights(EventLogEntry|EmailLogEntry $entry, bool $isCurrentUserAuthor = false): array
    {
        ...
        try {
            ...
            self::logError('Failed extracting structured highlights', $e);
        }
    ...
    public static function getUserDetails(?int $userId): ?array
    {
        ...
        try {
            ...
            self::logError("Failed getting user details for user {$userId}", $e);
        }
        ...
        try {
            ...
            if (!empty($entry->senderId)) {
                try {
                    ...
                    self::logError("Failed loading email sender user {$entry->senderId}", $e);
                }
            }
            ...
            self::logError('Failed getting email sender info', $e);
            return [
    ...
    public static function getFileInfoForLogEntry(EventLogEntry|EmailLogEntry $entry, bool $isCurrentUserAuthor = false): ?array
    {
        ...
        try {
            ...
            self::logError('Failed resolving file info for log entry', $e);
            return null;
        }
    }
}
